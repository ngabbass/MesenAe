...
const DisplayTab         = lazy(() => import('./settings/DisplayTab'));
...
type Tab = 'toko' | 'struk' | 'printer' | 'pembayaran' | 'pengguna' | 'tampilan' | 'data' | 'tentang';
...
const TABS: TabItem[] = [
  ...
  { 
    id: 'tampilan',    
    label: 'Tampilan',      
    description: 'Tema warna & gelap/terang.',
        ...
        >
          ...
          <div className="space-y-4">
            <Suspense key={activeTab} fallback={<div className="space-y-3"><Skeleton className="h-20 rounded-xl" /><Skeleton className="h-16 rounded-xl" /><Skeleton className="h-16 rounded-xl" /></div>}>
              ...
              {activeTab === 'tampilan'   && <DisplayTab hasEditAccess={hasEditAccess} />}
